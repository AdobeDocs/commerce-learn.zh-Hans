...
description: 了解Commerce集成入门工具包中源代码的组织​。
...
&#x200B;AEM
探索源代码结构背后的逻辑，详细说明每个实体文件夹下的`commerce`和`external`文件夹如何处理来自不同系统的事件。 此视频介绍`consumer`文件夹在将事件调度到相应的事件处理程序运行时操作以确保无缝处理方面所起的作用。 此视频还介绍了在运行时操作中实施的重试机制，以有效处理失败事件&#x200B;。了解Adobe Commerce Integration Starter Kit中源代码的组织和功能，从而针对事件处理、自动化脚本和配置设置提供有价值的分析。
...
* 了解不更改`starter-kit-info`文件夹中的代码的重要性，该文件夹包含Adobe用于基于入门工具包&#x200B;跟踪项目部署的运行时操作。
* 了解包含自动脚本（如`scripts`和`commerce-event-subscribe`）的`onboarding`文件夹，这些脚本可自动配置Commerce中的Adobe I/O Events模块、提供程序设置和配置&#x200B;。
