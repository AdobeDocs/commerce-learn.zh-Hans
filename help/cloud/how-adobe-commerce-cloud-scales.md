---
title: Adobe Commerce Cloud如何扩展
description: 了解Adobe Commerce Cloud项目如何扩展、哪些项目可自动扩展、哪些项目需要支持请求，以及如何规划事件或突发流量尖峰的容量。
feature: Cloud
topic: Commerce, Architecture
role: Admin, Architect, Leader
level: Intermediate
doc-type: Tutorial
duration: 285
last-substantial-update: 2026-03-26T00:00:00Z
jira: KT-15161
source-git-commit: 24edee9f9c5fbe3173e6d5a90ac3a84ed3809917
workflow-type: tm+mt
source-wordcount: '109'
ht-degree: 0%

---

# Adobe Commerce Cloud如何扩展

本教程介绍容量在Adobe Commerce Cloud上的工作方式、哪些内容会自动扩展、何时需要Adobe或支持，以及如何规划计划峰值与意外病毒流量。 录制Experience League视频时，请使用成绩单作为叙述脚本。

## 此视频面向谁？

* 解决方案架构师
* 云操作工程师
* 技术计划管理员

## 视频

>[!VIDEO](https://video.tv.adobe.com/v/3482046?captions=chi_hans&learn=on)

## 相关文档

* [云上的Commerce — 项目概述](https://experienceleague.adobe.com/zh-hans/docs/commerce-on-cloud/user-guide/project/overview)
* [性能测试指南](https://experienceleague.adobe.com/zh-hans/docs/commerce-on-cloud/user-guide/develop/test/guidance)
* [启动前核对清单](https://experienceleague.adobe.com/en/docs/commerce-learn/tutorials/cloud/best-practices/pre-launch-checklist)
