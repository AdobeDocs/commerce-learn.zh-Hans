---
title: 简化目录管理和促销
description: 使用Adobe Commerce从单个管理员面板管理产品数据、定价和促销规则，以实现更快、更智能的更新。
feature: Catalog Management, Attributes, Recommendations
topic: Commerce, Personalization
role: Developer, User
level: Beginner
doc-type: Technical Video
duration: 305
last-substantial-update: 2026-01-12T00:00:00Z
jira: KT-20088
source-git-commit: 86fb1a84f431726e282bd8af664dc668bdb5bc61
workflow-type: tm+mt
source-wordcount: '102'
ht-degree: 0%

---


# 简化目录管理和促销

了解Adobe Commerce as a Cloud Service如何简化目录管理和促销。 了解如何近乎实时地更新产品数据、定价和可见性规则，并使用智能推荐使购物体验个性化。

## 此视频面向谁？

* 电子商务促销商
* 产品经理
* 数字营销专家

## 视频内容

* 了解如何从集中式管理面板管理产品数据、属性和定价。
* 了解促销服务如何使用动态推荐和可见性规则使店面个性化。

>[!VIDEO](https://video.tv.adobe.com/v/3478794/?learn=on)
