---
title: Edge Delivery Services自定义签出
description: 了解如何使用Adobe Commerce组件和最佳实践自定义Edge Delivery Services签出，包括交付选项和多步签出。
feature: Best Practices, Compliance, Extensibility, Storefront, Tools and External Services
topic: Architecture, Commerce, Headless
role: Developer
level: Beginner
doc-type: Technical Video
duration: 885
last-substantial-update: 2025-01-02
jira: KT-16723
exl-id: e6b95ff0-9a41-43ac-9f02-c84a70747d64
TQID: https://experienceleague.adobe.com/sHAJy3AIwUgyWlGcViRb9ZT0ymIOap6HybG--NbI0jM
product_v2:
  - id: eadea719-cf89-469b-a6fd-a236a7138047
feature_v2:
  - id: e8818fe6-9c8b-4bc0-9ef8-377a10b7bc75
role_v2:
  - id: ff6a42d2-313e-452e-93a6-792e4fad9ff8
level_v2:
  - id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
topic_v2:
  - id: b5ce8718-c3af-4fdb-a1a9-fca32f83a87c
source-git-commit: d394484608d6ee530932387938e04030ed3b590e
workflow-type: tm+mt
source-wordcount: 143
ht-degree: 0%

---

# 自定义签出体验

了解如何在Adobe Commerce中通过Edge Delivery Services购物车拖放组件自定义结账体验，并侧重于修改配送选项和实施多步骤结账流程。

## 您将学习的内容

了解如何通过修改元素、实施多步骤流程并遵循最佳实践来自定义Edge Delivery Services购物车加货体验。

## 此视频面向谁？

* 使用Adobe Commerce、Edge Delivery Services并使用购物车的开发人员和技术专业人员

## 视频内容

* 签出体验的自定义
* 多步结账的实现
* 重用现有代码和最佳实践

>[!VIDEO](https://video.tv.adobe.com/v/3442660?captions=chi_hans&learn=on)

## 其他文档

* [多步结账](https://experienceleague.adobe.com/developer/commerce/storefront/dropins/checkout/tutorials/multi-step/?lang=zh-Hans)
