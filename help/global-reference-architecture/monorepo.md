...
* 支持编辑器支持的所有包类型
* 非常适用于临时环境，因为这些环境是可选的，但对于大批量投放团队来说，这些环境非常有用

缺点：

* 要部署未使用相同配置开发的软件包组合，需要严格的测试过程
* Monorepo GRA模式一开始可能很复杂。 指定一名领导来帮助团队处理系统
...
使用`bin/magento setup:install`安装Adobe Commerce。 提交生成的`app/etc/config.php`和编辑器文件。 Composer可管理任何其他内容，因此Git中没有任何其他内容。
...
要获取此示例中使用的元包和示例模块，请从GitHub上的[AntonEvers/gra-meta-foundation](https://github.com/AntonEvers/gra-meta-foundation)下载示例代码。
...
品牌隐喻需要基础隐喻。 当需要品牌隐喻时，基础隐喻也是自动需要的。 查看元包的两个composer.json文件以了解它们之间的关系：
...
使用中继包可以很好地组织属于同一部分的代码。 使用中继来定义区域、全球、品牌特定的程序包组或任何适合您的分组。 如果您维护多个Adobe Commerce安装，则使用程序包可以安全且全面地定义需要安装程序包的上下文。
...
Monorepo模块存在于`packages`目录中。 这样，Composer便可以通过路径类型存储库找到它们。
...
要启用已添加的模块，请运行`bin/magento module:enable --all`或仅针对特定模块运行。
...
1. [专用包程序](https://packagist.com/)
1. [简化Monorepo Builder](https://github.com/symplify/monorepo-builder)
1. 构建您自己的解决方案

[Private Packagist](https://packagist.com/)自动识别Git monorepo中的包，并通过Composer公开它们。 它与Adobe Commerce兼容，具有速度快、维护量低且不易出错等特点，这正是本指南重点介绍私有打包程序选项的原因。
...
定义可在monorepo中找到编辑器包的位置。 在示例中，位置为`packages/**/composer.json`。 要限制或扩大Private Packagist搜索要提取的包的位置，请更改位置。

包选项卡显示保存后找到的所有包，并且monorepo本身不再显示为Composer包：
...
如果您使用短时环境或计划使用它们，monorepo是一个很好的选择。 monorepo的每个版本和分支都包含所有Adobe Commerce、第三方和自定义模块文件。 通过在每个分支中进行完全安装，可以运行包括功能测试在内的每种类型的测试。 使用其他GRA设置（如单独的软件包或批量软件包GRA），在运行功能测试之前，首先构建一个有效的Adobe Commerce环境。 从DevOps的角度来看，monorepo可以简化操作过程。

## 代码示例

本文的代码示例已合并到一组Git存储库中，可用于试验概念验证。
