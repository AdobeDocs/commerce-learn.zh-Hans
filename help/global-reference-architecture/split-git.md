---
title: 拆分Git全局参考架构
description: 了解如何使用拆分Git全局参考架构来设置Adobe Commerce，以实现高效的代码管理和简化的部署。
jira: KT-16725
doc-type: Tutorial
duration: 330
...
* 您可以通过此模型安装任何编辑器包，包括模块、主题、语言包、composer-plugin、composer-metapackage、magento2-component和修补程序
...
* 在大多数情况下，批量打包模式是更好的替代方案，因为它同样简单，但更传统
...
这两个存储库存储第三方软件包和GRA软件包。 某些代码专用于Adobe Commerce的每个实例。 创建一个位置，将这些本地包存储在gra-split-brand-x存储库中。
...
Adobe Commerce是一个编辑器应用程序。 首选的安装方式始终通过Composer存储库。 仅当模块供应商不通过Composer存储库提供安装服务时，您才可以将第三方模块存储在第三方存储库中。 自定义代码的首选位置位于GRA存储库中。 当某个特定实例使用模块时，该模块将变为本地代码。
...
现在可以直接从`packages/3rdparty`、`packages/gra`和`packages/local`目录提交更改。 Git将更改提交到目录符号链接到的Git存储库。 在`packages/3rdparty`、`packages/gra`或`packages/local`目录中发出git commit命令很重要。 不要在项目根目录下运行git提交。
...
如果您看到上述输出，则可以将其安全地提交到品牌存储库。 要验证您是否正在提交到正确的远程数据库，请运行`git remote -v`。
...
要将代码交付到Adobe Commerce实例，请将GRA和第三方存储库合并到gra-split-brand-x存储库。 运行`composer require`、`bin/magento module:enable`并提交结果。
