...
Adding a module to the local code pool is simple. Download and extract the module. Composer需要它。 使用`bin/magento`启用它并在品牌存储库中提交文件。
...
将GRA和第三方存储库合并到gra-split-brand-x存储库，以将代码交付到Adobe Commerce实例。 Run `composer require`, `bin/magento module:enable` and commit the result.
