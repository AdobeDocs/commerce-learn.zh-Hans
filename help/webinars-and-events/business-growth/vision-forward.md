---
title: 使用Adobe的愿景展望路线图实现业务增长
description: 了解Adobe Commerce的B2B店面、实时个性化、支付以及AI支持的商家和开发人员体验路线图。
feature: App Builder,  B2B, Catalog Management,  Cloud,  Extensibility, Observability, Payments, Personalization, Storefront, Saas
topic: Administration, Artificial Intelligence,  Commerce,  Development, Headless,  Performance, Personalization, Security
old-role: Data Engineer, Developer
role: Developer
level: Beginner
doc-type: Technical Video
duration: 1016
last-substantial-update: 2025-10-10T00:00:00Z
jira: KT-19377
exl-id: a7f9ce6d-4bad-4fe2-8692-b8a02fc6dde6
source-git-commit: 9aa4d70ee6a3825f027aa2a9c6a1ac0f876ed59f
workflow-type: tm+mt
source-wordcount: '166'
ht-degree: 0%

---

# 使用Adobe的愿景展望路线图实现业务增长

Adobe Commerce2026年路线图概述了一项全面战略，旨在通过实时数据共享、个性化客户体验和简化运营方面的创新来促进业务增长。 关键开发包括与Adobe Experience Platform原生集成，以实现数据共享和个性化，以及快速B2B店面部署。 另外几个关键发展是扩展支付能力，以及重新构想的简化管理商人中心。 该路线图还强调了AI支持的用于目录管理、平台运行状况监控、通过App Builder实现的开发人员可扩展性以及可观察性增强的工具。 这些更新旨在跨B2B和B2C渠道提供无缝、可扩展和智能的商务体验。

## 此视频面向谁？

* Adobe Commerce开发人员
* 数字Commerce转型领导者
* 技术营销人员和商业工程师

## 视频内容

* Personalization的实时数据集成
* 原生营销活动编排
* 增强的B2B店面功能
* 商户中心和管理体验
* 开发人员体验和可观察性改进

>[!VIDEO](https://video.tv.adobe.com/v/3475716?captions=chi_hans&learn=on)
